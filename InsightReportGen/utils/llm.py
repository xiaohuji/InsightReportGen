...
from .validators import Subtopics, PipelinePlanResponse
import os
...
async def generate_pipeline_plan_prompt(
    ...
    **kwargs
    ...
    """
    try:
        parser = PydanticOutputParser(pydantic_object=PipelinePlanResponse)

        prompt = PromptTemplate(
            template=prompt_family.generate_pipeline_plan_prompt(),
            input_variables=[
                "user_requirement", "allowed_libs", "language",
                "output_dir", "figure_standards", "table_standards", "data_path"
            ],
            partial_variables={
                "format_instructions": parser.get_format_instructions()},
        )
        ...
        output = await chain.ainvoke({
            ...
            "allowed_libs": config.code_libs,
            "language": "python",
            "output_dir": output_dir,
            "figure_standards": config.figure_standards,
            "table_standards": config.table_standards,
        ...
        print("Exception in code requirement analysis : ", e)
        logging.getLogger(__name__).error("Exception in code requirement analysis : \n {e}")
...
async def code_llm(
        ...
        config,  # 你的 LLM 配置对象（含 provider、model、温度等）
        **kwargs
) -> str:
    # 1) 组织上下文
    plan_outline = "\n".join([f"- {s.id} {s.name}: {s.objective}" for s in plan.pipeline])
    step_json = json.dumps(step.dict() if hasattr(step, "dict") else step, ensure_ascii=False)

    # 2) PromptTemplate
    prompt = PromptTemplate(
        template=prompt_family.generate_code_prompt(),
        input_variables=["step_json", "plan_outline", "allowed_libs", "output_dir", "seed"]
    )

    provider_kwargs = {'model': config.smart_llm_model}
    if getattr(config, "llm_kwargs", None):
        provider_kwargs.update(config.llm_kwargs)
    if config.smart_llm_model in SUPPORT_REASONING_EFFORT_MODELS:
        provider_kwargs['reasoning_effort'] = ReasoningEfforts.High.value
    else:
...
async def fix_llm(
    ...
    **kwargs
    ...
    prompt = PromptTemplate(
        template=prompt_family.fix_code_prompt(),
        input_variables=["step_json", "plan_outline", "allowed_libs", "output_dir", "seed", "prev_code", "error_text"]
    )
...
async def revise_llm(
    ...
    **kwargs
    ...
    """
    返回一个 dict：
      - {"kind":"step","step": <Step对象或dict>}
      - {"kind":"plan","plan": <PipelinePlanResponse对象或dict>}
      - {"kind":"code","code": "<代码字符串>"}
    调用方可据此更新状态或直接追加代码 cell。
    """
    plan_outline = "\n".join([f"- {s.id} {s.name}: {s.objective}" for s in plan.pipeline])
    step_json = json.dumps(step.dict() if hasattr(step, "dict") else step, ensure_ascii=False)

    prompt = PromptTemplate(
        template=prompt_family.revise_plan_prompt(),
        input_variables=["step_json", "plan_outline", "allowed_libs", "output_dir", "seed", "error_text", "prev_code"]
    )
    ...
    # —— 解析严格 JSON —— #
    # 有些模型可能外层再包围了 markdown 代码块，做一层稳健处理
    ...
    try:
        ...
        # 若 JSON 解析失败，当做代码返回（兜底）
        return {"kind":"code","code": text}

    # 可在此处用 pydantic 做强校验（可选）：
    #   - if obj["kind"] == "step": Step.parse_obj(obj["step"])
    #   - if obj["kind"] == "plan": PipelinePlanResponse.parse_obj(obj["plan"])
    return obj